...
// Fetch all OJT users
$ojts = $oat->query("SELECT id, username, fname, lname, email, bio, profile_img, position, adviser_id FROM users WHERE role = 'ojt' ORDER BY lname, fname");
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <span class="badge rounded-pill"><?= $ojts->num_rows ?> OJTs</span>
        </div>
                ...
                <tbody>
                    <?php while ($ojt = $ojts->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($ojt['fname'] . ' ' . $ojt['lname']) ?></td>
            ...
            <div class="modal-content">
              <form id="manageForm" method="post" enctype="multipart/form-data">
                <div class="modal-header">
                  <h5 class="modal-title" id="manageModalLabel"><i class="bi bi-person-lines-fill"></i>Update OJT Profile</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="adviser_id" id="modal_adviser_id" class="form-control">
                            ...
                            ?>
                                <option value="<?= $trainer['id'] ?>"><?= htmlspecialchars($trainer['fname'] . ' ' . $trainer['lname']) ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                </div>
    ...
    <script>
    function openManageModal(id, fname, lname, username, email, bio, position, adviser_id) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_ojt'])) {
    ...
    $stmt = $oat->prepare("UPDATE users SET fname=?, lname=?, username=?, email=?, bio=?, position=?, adviser_id=? WHERE id=?");
    $stmt->bind_param("ssssssii", $fname, $lname, $username, $email, $bio, $position, $adviser_id, $ojt_id);
    $stmt->execute();
